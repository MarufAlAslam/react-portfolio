...
// jit photos
import jit1 from "./jit/1.png";
import jit2 from "./jit/2.png";
import jit3 from "./jit/3.png";
import jit4 from "./jit/4.png";
import jit5 from "./jit/5.png";
...
const ProjectDetails = () => {
  const projects = [
    {
      ...
      liveLink: "https://jadrooit.com",
      img: [jit1, jit2, jit3, jit4, jit5],
    },
    {
      id: 2,
      name: "PicMan",
  ...
  return (
    <div className="py-10 lg:w-5/6 w-full lg:px-0 px-4 mx-auto">
      <p className="text-3xl">{filteredProject[0].name}</p>

      <div className="projectDetails my-4">
        <ul>
          {filteredProject[0].projectDetails.map((detail, index) => (
      ...
      <a
        href={filteredProject[0].liveLink}
        className="text-primary text-2xl block mt-3"
      >
        Live Link
      </a>
      {filteredProject[0].clientLink && (
        <a
          href={filteredProject[0].clientLink}
          className="text-primary text-2xl block mt-3"
        >
          Client Side Repo Link
        </a>
      )}
      {filteredProject[0].serverLink && (
        <a
          href={filteredProject[0].serverLink}
          className="text-primary text-2xl block mt-3"
        >
      ...
      <p className="mt-8 mb-4 text-3xl">Screenshots</p>

      <div className="gallery grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6">
        {filteredProject[0].img.map((img, index) => (
